...
If in case, you already use `pyproject.toml` or you prefer to have that instead, just pass the flag `--format=pyproject` to the _init_ cmd earlier. You also need to do some extra steps if in case you go this way. (E.g., by prefixing each section with `tool.prefix` and including `build-system` section. Refer to [pixi's documentation](https://pixi.sh/latest/python/tutorial/#pixitoml-and-pyprojecttoml)) 
...
The whole `pixi.toml` now looks like this for me:
```toml
[workspace]
channels = ["conda-forge"]
name = "llm2"
...
Now we run a full fledged jupyter lab session by simply running `pixi run jupyter` at the commandline.
